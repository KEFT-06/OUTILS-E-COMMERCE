/**
 * Provenance d'un jeu de données affiché en graphique (feuille de route, 2.6).
 *
...
export interface DataProvenance {
  /** D'où viennent les chiffres : « Meta Ad Library », « Google Trends »… */
  source: string;
  /** Horodatage ISO de la collecte. */
  collectedAt: string;
  /** Volume d'échantillon. Sans lui, une proportion n'est pas interprétable. */
  sampleSize?: number;
  /** Ce que compte `sampleSize` : « publicités », « requêtes », « annonceurs ». */
  sampleUnit?: string;
  /**
...
/** Provenances rattachées aux blocs chiffrés d'un rapport. */
