import assert from 'node:assert/strict';
import { after, before, describe, it } from 'node:test';
import type { Express } from 'express';
import { type FakeProviders, signInWithPlan, startFakeProviders } from './analysis-fixtures';
import { closeTestApp, createTestApp } from './helpers';

/**
 * Analyse de niche sans recherche web, avec la collecte publicitaire de
 * démonstration : la saturation est mesurée et étiquetée comme démonstration, et
 * aucun fait de marché n'est avancé, même si le modèle en propose.
 */

let app: Express;
let providers: FakeProviders;

before(async () => {
  providers = await startFakeProviders();
  app = await createTestApp({ GEMINI_API_URL: `${providers.base}/gemini`, AD_INGESTION_ADAPTER: 'fixture' });
});

after(async () => {
  await closeTestApp();
  await providers.close();
});

describe('Analyse de niche sans recherche web', () => {
  it('mesure la saturation, n’avance aucun fait sans source et le dit', async () => {
    const { agent } = await signInWithPlan(app, 'sans-web@exemple.com', 'pro');
    const response = await agent.post('/api/analyze-niche').send({ query: 'formation Excel' }).expect(201);
    const report = response.body as {
      overallVerdict: string | null;
      verdictRationale: string;
      competitors: unknown[];
      rates: Record<string, { basis: string; score: number | null; trace?: { source: string; breakdown: unknown[] } }>;
      groundingSources: unknown[];
      limitations: string[];
      digitalProducts: { pricingNote: string; currency: string }[];
      dataProvenance: { rates?: { isDemonstration: boolean } };
    };

    assert.equal(providers.braveQueries.length, 0, 'aucune recherche web sans clé');
    assert.match(providers.geminiCalls.at(-1)!.prompt, /la recherche web n’est pas branchée/);

    const saturation = report.rates.saturation!;
    assert.equal(saturation.basis, 'measured');
    assert.equal(typeof saturation.score, 'number');
    assert.equal(saturation.trace?.source, 'demonstration');
    assert.equal(saturation.trace?.breakdown.length, 4);
    assert.equal(report.dataProvenance.rates?.isDemonstration, true);

    assert.deepEqual(report.competitors, [], 'le concurrent proposé par le modèle est écarté faute de source');
    assert.deepEqual(report.groundingSources, []);
    for (const key of ['demand', 'profitability', 'opportunity', 'virality']) {
      assert.equal(report.rates[key]!.basis, 'unavailable', key);
    }
    assert.equal(report.overallVerdict, null);
    assert.match(report.verdictRationale, /aucune recherche web/);
    assert.match(report.digitalProducts[0]!.pricingNote, /faute de recherche web/);
    assert.equal(report.digitalProducts[0]!.currency, 'USD', 'sans pays, devise par défaut');
    assert.ok(report.limitations.some((limitation) => limitation.includes('Aucune recherche web')));
    assert.ok(report.limitations.some((limitation) => limitation.includes('démonstration')));
  });
});
